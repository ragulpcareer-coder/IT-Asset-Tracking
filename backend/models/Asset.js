...
// NOTE: serialNumber, assignedTo, ipAddress, and macAddress are
// intentionally NOT field-encrypted. This is a deliberate correction, not
// an oversight: mongoose-field-encryption only supports searching
// encrypted fields when every query manually pre-encrypts the search term
// first (via encryptFieldsSync()) — nothing in this codebase did that,
// and the free-text serial/assignee search (a regex match) cannot work
// against ciphertext at all, encrypted or not. With these fields
// encrypted, every plaintext query against them (asset search,
// bulk-import duplicate detection, "my assets" self-service filtering,
// and — most seriously — the network-discovery job's own lookup by
// MAC/IP to recognize an already-known device) silently matched nothing,
// so the discovery job re-created every known device as a brand-new
// "rogue" asset on every single scan cycle instead of recognizing it.
// None of these values are sensitive secrets the way a password or TOTP
// seed is, so leaving them as plain (but still access-controlled) fields
// is the correct tradeoff, not a security regression.
