...
if (process.env.EMAIL_VERIFY_ON_BOOT === "true") {
try {
...
const fromEmail = process.env.EMAIL_FROM || "IT Asset Tracker <no-reply@example.com>";
...
const sendSecurityAlert = async (subject, message, recipientEmail = null) => {
    const adminEmail = recipientEmail || process.env.ADMIN_EMAIL;
    await sendEmail({
        to: adminEmail,
        subject: `SECURITY ALERT: ${subject}`,
        html: `
            <div style="font-family: Arial, sans-serif; padding: 20px; border: 1px solid #ff0000; border-radius: 5px;">
                <h2 style="color: #ff0000;">Security Incident Detected</h2>
    ...
    return await sendEmail({
        ...
        html: `
            <div style="font-family: Arial, sans-serif; padding: 20px; border: 1px solid #00d4ff; border-radius: 8px; max-width: 600px; margin: auto; background-color: #0d1117; color: #ffffff;">
                ...
                <p style="font-size: 13px; color: #8b949e;">This link expires in 15 minutes. If you did not request this, you can safely ignore this email.</p>
            </div>
        `
    });
};
...
const sendWelcomeOnboardingEmail = async ({ name, email, asset }) => {
    const frontendUrl = resolveFrontendUrl();
    const assetLink = `${frontendUrl}/assets`;

    return await sendEmail({
        to: email,
        subject: "Welcome to AssetTrack - Your Device Is Ready",
        html: `
