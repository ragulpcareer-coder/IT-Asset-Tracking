...
exports.getSecurityStatus = async (req, res) => {
    try {
        ...
        const lockedAccounts = 0; // Account lockout is no longer tracked (simplified auth model)
...
exports.triggerManualBackup = async (req, res) => {
    try {
        ...
        // Still write a local copy (useful if the process stays up and you
        // want to re-download later via /backup/download/:filename), but
        // this is NOT durable storage — on any host with ephemeral disks
        // (Render, Heroku, most container platforms), this file is gone on
        // the next restart or redeploy. That's why the encrypted backup is
        // also streamed directly to the browser below, in the same request
        // that creates it — so there's no window where the only copy exists
        // solely on local disk waiting for someone to remember to fetch it.
        fs.writeFileSync(backupFile, encrypted);

        await AuditLog.create({
            action: "SECURITY: Manual System Backup",
            performedBy: req.user.email,
            details: `Admin-initiated encrypted database backup created and downloaded: ${filename}.`,
            ip: req.ip || req.connection?.remoteAddress
        });
