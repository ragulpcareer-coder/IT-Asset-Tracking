...
const createAsset = async (req, res) => {
  if (!req.user || !["Super Admin", "Admin", "Asset Manager"].includes(req.user.role)) {
    return sendError(res, 403, "You do not have permission to create assets.", {
      asset: "forbidden",
    });
  }
  try {
...
const updateAsset = async (req, res) => {
  if (!req.user || !["Super Admin", "Admin", "Asset Manager"].includes(req.user.role)) {
    return sendError(res, 403, "You do not have permission to edit this asset.", {
      asset: "forbidden",
    });
  }
  try {
    ...
    return sendError(res, 500, "Something went wrong updating this asset. Please try again.", {
      asset: "update_failed",
    });
  }
};
...
const getSecurityAlerts = async (req, res) => {
  try {
    ...
    dbAlerts.forEach((alert) => {
      try {
        ...
        alerts.push({
          ...
          issue: alert.message || alert.description || "Access denied",
    ...
    return sendError(res, 500, "Something went wrong loading security data. Please try again.", {
      alerts: "fetch_failed",
    });
  }
};
...
const verifyAssetIntegrity = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Could not verify asset integrity due to a server error." });
  }
};
...
const getPublicAssetHealth = async (req, res) => {
  try {
    ...
    const warrantyState = !warrantyExpiry
      ? "unknown"
