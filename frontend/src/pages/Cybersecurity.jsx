        ...
        <div className="fade-in pb-12">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
                <div>
                    ...
                    <p className="text-slate-500 font-medium mt-1 text-xs tracking-widest uppercase italic pt-1">
                        Zero-Trust Network Discovery & Rogue Node Detection
                    </p>
                </div>
                <Button
                    variant="danger"
                ...
                >
                    {scanning ? "Discovering Network..." : "🔍 Run Discovery Protocol"}
                </Button>
            </div>

            {/* Dynamic Scan Logic Alert */}
            {scanResult && (
                <Card className="mb-8 border-red-500/30 bg-red-500/10 p-4 border-l-4">
                    <div className="flex justify-between items-center">
                        <div>
                            <div className="text-xs font-black text-red-500 uppercase tracking-widest mb-1">Discovery Telemetry Result</div>
                            <div className="text-sm font-bold text-white mb-2">{scanResult.message}</div>
                            {scanResult.device && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
                <Card className="border-red-500/20 bg-slate-900/40 border-l-[6px] border-l-red-500">
                    <div className="text-[10px] font-black uppercase text-slate-500 tracking-widest mb-1">Critical Threat Delta</div>
                    <div className="text-4xl font-black text-red-500 tabular-nums">
                        {alerts.filter(a => a.securityStatus?.riskLevel === 'High' || a.securityStatus?.riskLevel === 'Critical').length}
                    </div>
                    <div className="text-[10px] text-red-400 font-bold mt-2 italic uppercase">Action Required Immediately</div>
                </Card>
                <Card className="border-amber-500/20 bg-slate-900/40 border-l-[6px] border-l-amber-500">
                    <div className="text-[10px] font-black uppercase text-slate-500 tracking-widest mb-1">Rogue Node Vectors</div>
                    <div className="text-4xl font-black text-amber-500 tabular-nums">
                        {alerts.filter(a => a.securityStatus?.isAuthorized === false).length}
                    </div>
                    <div className="text-[10px] text-amber-400 font-bold mt-2 italic uppercase">Unauthorized Registry Entrance</div>
                </Card>
                <Card className="border-cyan-500/20 bg-slate-900/40 border-l-[6px] border-l-cyan-500">
                    <div className="text-[10px] font-black uppercase text-slate-500 tracking-widest mb-1">Adversarial IQ Shield</div>
                    <div className="text-4xl font-black text-cyan-500 tabular-nums">
                        {auditAlerts.filter(l => l.action?.includes('AI') || l.action?.includes('Injection')).length}
                    </div>
                    <div className="text-[10px] text-cyan-400 font-bold mt-2 italic uppercase">Prompt Injections Neutralized</div>
                </Card>
                <Card className="border-emerald-500/20 bg-slate-900/40 border-l-[6px] border-l-emerald-500">
                    <div className="text-[10px] font-black uppercase text-slate-500 tracking-widest mb-1">Zero Trust Health</div>
                    <div className="text-4xl font-black text-emerald-500 tabular-nums">100%</div>
                    <div className="text-[10px] text-emerald-400 font-bold mt-2 italic uppercase">Identity Validation Active</div>
                </Card>
            </div>


            {/* MITRE ATT&CK Intelligence Feed (§Category 3) */}
            {auditAlerts.some(l => l.action?.includes('DETECTION')) && (
                <div className="mb-10">
            ...
            <Card className="p-0 overflow-hidden border-white/5 bg-slate-900/40">
                ...
                {loading ? (
                    ...
                    <div className="text-center py-20 text-slate-500 font-bold uppercase tracking-widest text-sm italic">
                        No immediate anomalies detected on local bridge.
                    </div>
                ) : (
