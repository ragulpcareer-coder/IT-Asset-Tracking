...
import { Button, Card, Badge, PasswordStrengthMeter, Alert, ConfirmModal } from "../components/UI";
...
export default function Settings() {
    ...
    <motion.div className="space-y-6 max-w-2xl" initial="hidden" animate="visible" variants={animationVariants.containerVariants}>
      <motion.div variants={animationVariants.itemVariants}>
        ...
        {["Super Admin", "Admin"].includes(user?.role) && (
          <motion.div variants={animationVariants.itemVariants} className="mt-6 pt-6 border-t border-white/10">
            <h3 className="text-sm font-semibold text-white mb-2">System Backup</h3>
            <p className="text-xs text-gray-500 leading-relaxed mb-3">
              Downloads an encrypted snapshot of users, assets, and audit logs directly to your
              browser. Save this file somewhere durable (your computer, a cloud drive) — it is
              not kept anywhere else once downloaded.
            </p>
            <button
              className="w-full bg-white/5 hover:bg-white/10 text-white border border-white/10 rounded-lg py-3 font-semibold transition flex items-center justify-center gap-2 disabled:opacity-50"
              disabled={backupLoading}
              onClick={async () => {
                setBackupLoading(true);
                try {
                  const res = await axios.post("/maintenance/backup", {}, { responseType: "blob" });
                  const url = window.URL.createObjectURL(new Blob([res.data]));
                  const link = document.createElement("a");
                  link.href = url;
                  const stamp = new Date().toISOString().replace(/[:.]/g, "-");
                  link.download = `manual-backup-${stamp}.json`;
                  document.body.appendChild(link);
                  link.click();
                  link.remove();
                  window.URL.revokeObjectURL(url);
                  toast.success("Backup downloaded. Store the file somewhere safe.");
                } catch (e) {
                  toast.error(e.response?.data?.message || "Backup failed. Please try again.");
                } finally {
                  setBackupLoading(false);
                }
              }}
            >
              {backupLoading ? "Preparing backup..." : "📦 Download Encrypted Backup"}
            </button>
          </motion.div>
        )}
      </motion.div>
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  const ActivityTab = () => {
    const filterOptions = ["All", "Security", "Login", "Profile", "Asset"];
