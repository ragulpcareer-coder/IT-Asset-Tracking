...
const STEP_COPY = {
  1: {
    ...
    description: "Start with the basics so we can create your account request.",
  },
  2: {
  ...
  3: {
    title: "Review & Submit",
    description: "Confirm your details before sending the registration request.",
  },
};
...
export default function Register() {
  ...
  const handleNext = async () => {
    ...
    if (activeStep === 1) {
      ...
      try {
        ...
        setFieldError(
          "email",
          err.response?.data?.message || "Unable to validate email right now. Please try again."
        );
      } finally {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      redirectTimerRef.current = setTimeout(() => navigate("/login"), 1800);
    } catch (err) {
    ...
    <AuthShell
      ...
      footer={
        <div className="space-y-3">
          ...
          <p className="text-xs text-slate-500">Your role is assigned after approval by an administrator.</p>
        </div>
      }
    >
